...
const CustomizeProduct = ({
  ...
  const [selectVarient, setselectVarient] = useState<products.Variant>();
    ...
    <div className="flex flex-col gap-6">
      {productOptions.map((option) => {
        return (
          <div className="flex flex-col gap-4" key={option.name}>
            ...
            <ul className="flex items-center gap-3">
              {option.choices?.map((choice) => {
                ...
                const clickHadler = disabled
                ? undefined
                : () => handleOptionsSelected(option.name!, choice.description!)

                return option.name === "Cor" ? (<li className="w-8 h-8 rounded-full ring-1 ring-gray-300 relative "
          ...
          <Add productId={productId} variantId={selectVarient?._id ||"00000000-0000-0000-0000-000000000000"} stockNumber={selectVarient?.stock?.quantity || 0}/>
    </div>
  )
